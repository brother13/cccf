...
class Cfcc extends Common
{
    ...
    protected function __init()
    {
        ...
        $table  = [];
        $table['code'] = 'fc'; // 字符
        $table['table'] = "admin_cflist"; // 表名
        $table['name'] = "房产";

        $field = [];
        $field[] = ['field' => 'ah', 'name' => '案号', 'show' => false, "filter" => true];
        $field[] = ['field' => 'cbr', 'name' => '承办人', "width" => 150, "filter" => true];
        $field[] = ['field' => 'sqzxr', 'name' => '申请执行人', "filter" => true, "width" => 200];
        $field[] = ['field' => 'bzxr', 'name' => '被执行人', "filter" => true, "width" => 200];
        $field[] = ['field' => 'startdate', 'name' => '查封开始日期', "filter" => true];
        $field[] = ['field' => 'enddate', 'name' => '查封届满日期', "filter" => true];
        $field[] = ['field' => 'note', 'name' => '财产详细信息', "filter" => true];
        $field[] = ['field' => 'type', 'name' => '财产类型', "filter" => true];
        $table['field'] = $field;
     ...
     * @return [type]         [description]
     */
    public function saveCfcc($data){
        $field=['ah','cbr','sqzxr','bzxr','startdate','enddate','note','type'];
        $fieldName=['案号','承办人','申请人','被申请人','查封开始日期','查封届满日期','财产详细信息','财产类型'];
        $jsondata=json_decode($data);
        $cnum=0;
