...
class Thqd extends Common
{
    // 表名（不含前缀，前缀由数据库配置自动添加）
    const TABLE_NAME = 'thqd';
    ...
    public function getList($param = [])
    {
        ...
        // 默认只查询当前用户的数据
        // 如果用户有 THQD_QUERY_ALL 权限，则可以查询所有人的数据
        $canQueryAll = $this->checkAuth('THQD_QUERY_ALL');
        if (!$canQueryAll) {
        ...
        // 收款人查询
        if (!empty($param['skr'])) {
            $where['skr'] = ['like', '%' . $param['skr'] . '%'];
        }

        // 出账状态查询
        if (!empty($param['czzt'])) {
            $where['czzt'] = $param['czzt'];
        }
