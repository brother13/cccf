...
class User extends Common
{
    ...
    public function getUserinfo($id)
    {
        ...
        $_config = $this->getConfiginfo($userinfo['dwid'],$userinfo['deptcode']);
     ...
     * @return void
     */
    protected function getConfiginfo($dwid = '', $deptcode = "all")
    {
        $where = [];
        $where['dwid'] = $dwid;
        $defaultconfig = null;//默认的配置
        $info = $this->getdb("config")->where($where)->select();
        foreach ($info as $row) {
            if ($deptcode == $row['deptcode']) {
                return $row;//有对应部门则取指定单位
            }
            if ('all' == $row['deptcode']) {
                $defaultconfig = $row;//无指定部门则取默认值
            }
        }
